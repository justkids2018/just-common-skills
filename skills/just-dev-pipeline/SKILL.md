...
每个 Task 都必须形成可审计证据，至少包含：

1. `SPEC.md`：任务合同（可引用 `01-requirement.md` + `04-tasks.md` 提炼）。
2. `solution.*`：当前 Task 的实现结果。
3. `RUN_LOG.md`：build/test/run 的命令与结果证据。
4. `DIAGNOSIS.md`：仅在失败时必须产出，且先诊断再大改。

推荐目录：`doc/features/<feature>/runs/<task-id>/`

默认收尾动作（强制）：每个 Task 完成后，必须执行
[`system-platform/06-compliance-checklist.md`](../../system-platform/06-compliance-checklist.md)
中的 Task-Level Checklist。
...
1. 只要发生代码改动（新增功能、修复问题、交互调整），默认在实现后调用 `just-qa`。
2. 若涉及 Flutter/Android/iOS UI 改动，`just-qa` 必须执行页面定位、截图、交互与布局检查。
3. 仅当用户明确要求“先分析不执行”或“本次先不做 QA”时，可跳过自动 QA。
4. 若环境缺失导致无法执行 QA，必须输出 `BLOCKED` 并列出所需环境与下一动作。
...
2. 每 Task 完成后：
   ...
   - 若涉及 UI 改动，必须在 `just-qa` 中执行页面定位、截图、交互与布局检查
   - 若验证失败，调用 `just-investigate` 先做根因定位，再进入修复
   - 根据运行结果持续小步修改，直到当前 Task 可用
   - 在 `doc/features/<feature>/runs/<task-id>/` 更新 `solution.*` 与 `RUN_LOG.md`
   - 若涉及 UI 改动，在 `doc/features/<feature>/runs/<task-id>/ui/` 保存验收截图
   - 若失败，必须生成或更新 `DIAGNOSIS.md`
   - 若需求发生变化，立即更新 `doc/features/<feature>/01-requirement.md` 与 `04-tasks.md`
   - 简报：改了哪些文件、行数、测试结果
   - **任何模式都停下等"继续"**（fast 不等需求/设计/拆分，但每个 Task 都等）
3. 编码必须遵守：
   - [`common-prompt/baseline/03-coding-standards.md`](../../common-prompt/baseline/03-coding-standards.md)
   - [`common-prompt/baseline/04-testing-standards.md`](../../common-prompt/baseline/04-testing-standards.md)
4. **build/test 失败立即停下**，禁止"先跳过这个测试"
5. 测试结果落盘到 `doc/features/<feature>/05-test-result.md`。
6. 当前 Task 的运行证据落盘到 `doc/features/<feature>/runs/<task-id>/RUN_LOG.md`。
7. 涉及 UI 改动时，截图证据落盘到 `doc/features/<feature>/runs/<task-id>/ui/`。
...
6. 若经历失败修复，必须检查 `doc/features/<feature>/runs/<task-id>/DIAGNOSIS.md` 是否完整。
...
- baseline 标准目录：`common-prompt/baseline/`
- 任何冲突：baseline 优先
